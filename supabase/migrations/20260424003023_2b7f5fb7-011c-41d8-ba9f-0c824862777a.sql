DROP FUNCTION IF EXISTS public.get_chamadas_recentes_detalhadas(uuid);

CREATE OR REPLACE FUNCTION public.get_chamadas_recentes_detalhadas(_unidade_id uuid)
 RETURNS TABLE(id uuid, senha_id uuid, unidade_id uuid, destino text, created_at timestamp with time zone, senha_codigo text, fila_nome text, paciente_nome text, triagem_dados jsonb)
 LANGUAGE sql
 STABLE
AS $function$
  SELECT 
