CREATE OR REPLACE FUNCTION public.gerar_senha(
  _fila_id uuid,
  _prioridade public.senha_prioridade DEFAULT 'normal'::senha_prioridade,
  _paciente_id uuid DEFAULT NULL::uuid,
  _origem text DEFAULT 'recepcao'::text
)
...
BEGIN
  ...
  -- Permissão: admin, recepção ou super_admin da unidade da fila
  IF NOT (
    ...
    OR public.has_role_in_unidade(_user_id, _fila.unidade_id, 'super_admin'::app_role)
  ) THEN
