-- Adiciona coluna criado_por na tabela senhas
ALTER TABLE public.senhas ADD COLUMN criado_por UUID REFERENCES auth.users(id);

-- Atualiza a função gerar_senha para salvar o ID do usuário
...
BEGIN
  ...
  -- Permissão: admin, recepção, gestor da unidade ou super_admin global
  IF NOT (
    _is_super_admin
