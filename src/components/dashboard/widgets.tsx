...
function StatCard({
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-border/60 bg-card p-4 sm:p-6 shadow-soft transition-all duration-300 hover:-translate-y-1 hover:shadow-elegant animate-in fade-in slide-in-from-bottom-2">
      ...
      <div className="flex items-center justify-between">
        <div className={`flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-xl shadow-sm transition-transform group-hover:scale-110 group-hover:rotate-3 ${accentBg}`}>
      ...
      <div className="mt-4 sm:mt-6">
        <div className="font-display text-2xl sm:text-3xl font-bold tracking-tight">
          {loading ? <Skeleton className="h-8 w-20 sm:h-9" /> : value}
        </div>
        <div className="mt-1 flex items-center gap-1.5 font-bold text-muted-foreground/80 group-hover:text-foreground transition-colors">
          <span className="text-[10px] sm:text-xs uppercase tracking-widest">{label}</span>
        </div>
        {hint && <div className="mt-1 text-[10px] sm:text-xs text-muted-foreground/60 line-clamp-1">{hint}</div>}
      </div>
    </div>
  );
}
...
/**
 * Pede permissão para notificações do navegador (idempotente).
 * Só pede se o estado for "default" — não reabre o prompt se já foi negado.
 */
...
/**
 * Mostra uma notificação nativa apenas se a aba NÃO estiver visível
 * (background, minimizada ou em outra aba). Se a aba estiver focada,
 * o toast + som já são suficientes.
 */
function notifyUrgenteIfHidden(codigo: string, fila: string | null) {
  if (typeof window === "undefined" || !("Notification" in window)) return;
  if (Notification.permission !== "granted") return;
  if (typeof document !== "undefined" && document.visibilityState === "visible") return;

  try {
    const n = new Notification(`🚨 Senha urgente: ${codigo}`, {
      body: fila ? `Nova prioridade urgente em ${fila}` : "Nova prioridade urgente na fila",
      tag: `senha-urgente-${codigo}`, // evita empilhar notificações duplicadas
